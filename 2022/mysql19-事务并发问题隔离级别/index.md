...
MySQL的事务隔离级别，是由各种**锁**和**MVCC机制**（Multi-Version Concurrency Control 多版本并发控制）实现的。



## 简述事务隔离性和隔离级别

实际项目中涉及到很多的事务，基于效率考虑，要允许事务的并发执行，并发执行的事务必然涉及到数据的安全性和一致性问题。事务并发问题，主要有，脏读、不可重复读、虚读（幻读）等。为了在不同程度上（数据安全性和一致性、并发的效率）解决这些问题，所以引入了事务的隔离级别。

串行化级别下，并发的效率最低，数据的安全性最高。串行化主要由锁机制来实现，简单来说就是，给每个事务排序，依次执行。

未提交读级别下，并发的效率最高，数据的安全性最低。该级别下，没有做任何的并发控制。

MySQL中，综合考虑了数据安全性一致性和并发的效率问题，采用各种锁机制和MVCC多版本并发控制机制，默认情况下，使用可重复读的隔离级别。既在一定程度上保证数据安全，也允许相当程度的并发度。
